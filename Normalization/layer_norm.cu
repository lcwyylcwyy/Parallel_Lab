...
__global__ void layer_norm_f32_kernel(float* x, float* y, float* g, float* b, int N, int K) {
  ...
  __syncthreads();
  if (idx < N * K) y[idx] = ((value - s_mean) * s_variance) * gamma + bias;
}


template<const int NUM_THREADS=256>
__global__ void layer_norm_f32_kernel_backward(float* dx, float* dgama, float* dbias, float* d_out, float* x, float* g, float* b, int N, int K) {
  int tid = threadIdx.x; // 0..K-1
  int bid = blockIdx.x; // 0..N-1
  int idx = bid * blockDim.x + threadIdx.x;
  const float epsilon = 1e-5f;
...
__global__ void layer_norm_f32x4_kernel(float* x, float* y, float* g, float* b, int N, int K) {
  ...
  float4 gamma4 = (tid < K / 4) ? FLOAT4(g[tid*4]): zero4;
  float4 bias4 = (tid < K / 4) ? FLOAT4(b[tid*4]): zero4;

  reg_y.x = reg_x_hat.x * s_variance * gamma4.x + bias4.x;
  reg_y.y = reg_x_hat.y * s_variance * gamma4.y + bias4.y;
  reg_y.z = reg_x_hat.z * s_variance * gamma4.z + bias4.z;
  reg_y.w = reg_x_hat.w * s_variance * gamma4.w + bias4.w;
...
layer_norm_f32_kernel<(K)><<<grid, block>>>( \
  ...
  reinterpret_cast<float*>(b.data_ptr()), N, (K));  
...
void layer_norm_f32(torch::Tensor x, torch::Tensor y, torch::Tensor g, torch::Tensor b) {
...
void layer_norm_f32x4(torch::Tensor x, torch::Tensor y, torch::Tensor g, torch::Tensor b) {
  CHECK_TORCH_TENSOR_DTYPE(x, torch::kFloat32)       
  CHECK_TORCH_TENSOR_DTYPE(y, torch::kFloat32)
  CHECK_TORCH_TENSOR_SHAPE(x, y)
  const int N = x.size(0);
