...
__global__ void rms_norm_f32_kernel(float* x, float* y, float* g, int N, int K) {
  ...
  float gamma = (tid < K) ? g[tid] : 1.0f; 
  float sum_pow = block_reduce_sum_f32<NUM_THREADS>(value * value);
  if (tid == 0) {
    // printf("idx=%d, s_variance=%f\n", idx, s_variance);
    s_variance = rsqrt(sum_pow / (float) K + epsilon);
  }
  // wait for s_variance in shared memory to be ready for all threads
  __syncthreads();
  if (idx < N * K) y[idx] = (value * s_variance) * gamma;
}


template<const int NUM_THREADS=256>
__global__ void rms_norm_f32_kernel_backward(float* dx, float* dgama, float* dbias, float* d_out, float* x, float* g, float* b, int N, int K) {
  int tid = threadIdx.x; // 0..K-1
  int bid = blockIdx.x; // 0..N-1
  int idx = bid * blockDim.x + threadIdx.x;
  ...
  __shared__ float s_mean_diff; // shared within block
  __shared__ float s_variance; // shared within block
  float value = (idx < N * K) ? x[idx] : 0.0f; // load once only
  float gamma = (tid < K) ? g[tid] : 0.0f; 
  float bias = (tid < K) ? b[tid] : 0.0f; 
  float dx_hat = (idx < N * K) ? d_out[idx] * gamma : 0.0f;
  ...
  float v2 = (value - s_mean);
  float variance = v2 * v2;
  variance = block_reduce_sum_f32<NUM_THREADS>(variance);
  if (tid == 0) s_variance = rsqrtf(variance / ((float) K + epsilon));
  // wait for s_variance in shared memory to be ready for all threads
  __syncthreads();

  float dvar =  dx_hat * v2 * -0.5f * powf((s_variance+epsilon), -1.5f);
  dvar = block_reduce_sum_f32<NUM_THREADS>(dvar);
  __syncthreads();
  float dmean = dx_hat * -1 / sqrtf(s_variance + epsilon);
  dmean = block_reduce_sum_f32<NUM_THREADS>(dmean);
  __syncthreads();
  float mean_diff = -2 * v2;
  float mean_diff_sum = block_reduce_sum_f32<NUM_THREADS>(mean_diff);
  if (tid == 0) s_mean_diff = mean_diff_sum / (float) K;
  dmean += (dvar * s_mean_diff);
  __syncthreads();
  if (idx < N * K) dx[idx] = dx_hat / sqrtf(s_variance + epsilon) + dvar * 2 * v2 / N + dmean / N;
  __syncthreads();
  float d_bias = d_out[idx];
  if (tid < K) dbias[tid] = block_reduce_sum_f32<NUM_THREADS>(d_bias);
  __syncthreads();
  float d_gama = d_out[idx] * dx_hat;
  if (tid < K) dgama[tid] = block_reduce_sum_f32<NUM_THREADS>(d_gama);
  __syncthreads();
}


// rms Norm Vec4: x: NxK(K=256<1024), y': NxK, y'=x-mean(x)/std(x) each row
// mean(x) = sum(x)/K, 1/std(x) = rsqrtf( sum( (x-mean(x))^2 )/K ) each row
...
__global__ void rms_norm_f32x4_kernel(float* x, float* y, float* g, int N, int K) {
  ...
  float value = (idx < N * K) ? (reg_x.x * reg_x.x + reg_x.y * reg_x.y 
                                + reg_x.z * reg_x.z + reg_x.w * reg_x.w) : 0.0f;
  float sum = block_reduce_sum_f32<NUM_THREADS>(value);
  if (tid == 0) s_variance = rsqrtf(sum / (float) K + epsilon);
  // wait for s_mean in shared memory to be ready for all threads
  __syncthreads();
  float4 reg_y;
  float4 one4 = {1.0f, 1.0f, 1.0f, 1.0f};
  float4 gamma4 = (tid < K / 4) ? FLOAT4(g[tid*4]): one4;

  reg_y.x = reg_x.x * s_variance * gamma4.x;
  reg_y.y = reg_x.y * s_variance * gamma4.y;
  reg_y.z = reg_x.z * s_variance * gamma4.z;
  reg_y.w = reg_x.w * s_variance * gamma4.w;
...
void rms_norm_f32(torch::Tensor x, torch::Tensor y, torch::Tensor g) {
...
void rms_norm_f32x4(torch::Tensor x, torch::Tensor y, torch::Tensor g) {
  CHECK_TORCH_TENSOR_DTYPE(x, torch::kFloat32)       
  CHECK_TORCH_TENSOR_DTYPE(y, torch::kFloat32)
  CHECK_TORCH_TENSOR_SHAPE(x, y)
  const int N = x.size(0);
