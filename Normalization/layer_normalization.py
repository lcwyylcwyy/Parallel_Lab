...
def validate_numpy_layer_norm():
    ...
    # 对比
    print("Forward diff:", np.allclose(out_np, out.detach().numpy(), atol=1e-5))
    print("dx diff:", np.allclose(dx_np, x.grad.numpy(), atol=1e-5))
    print("dgamma diff:", np.allclose(dgamma_np, layer_norm.weight.grad.numpy(), atol=1e-4))
    print("dbeta diff:", np.allclose(dbeta_np, layer_norm.bias.grad.numpy(), atol=1e-4))
